...
import { confirm_reg_cancellationComponent } from 'app/components/PopUps/confirm_reg_cancellation.component'; //_splitter_
...
export class unable_to_captureComponent {
  ...
  confirmCancellation(...others) {
    ...
    try {
      ...
      bh.input = {};
      bh.local = {};
      bh = this.sd_dSEIhIWVNJiinHUn(bh);
      //appendnew_next_confirmCancellation
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_G1OhHhaA1w1sBiSu');
    }
  }
  ...
  sd_dSEIhIWVNJiinHUn(bh) {
    try {
      const confirm_reg_cancellationDialog =
        this.__page_injector__.get(MatDialog);
      const confirm_reg_cancellationDialogRef =
        confirm_reg_cancellationDialog.open(confirm_reg_cancellationComponent, {
          width: '350px',
        });

      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_dSEIhIWVNJiinHUn');
    }
  }
