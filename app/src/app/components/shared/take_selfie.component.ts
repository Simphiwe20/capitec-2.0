...
export class take_selfieComponent implements AfterViewInit {
  ...
  storeUserInfor(url: any = undefined, infor: any = undefined, ...others) {
    let bh: any = {};
    try {
      bh = this.__page_injector__
        .get(SDPageCommonService)
      ...
      bh.input = { url, infor };
      bh.local = {};
      bh = this.sd_758Wv0M4eHNK7wFL(bh);
      //appendnew_next_storeUserInfor
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_b4CBC4MNxIINIcYt');
    }
  }
  ...
  async getUserInfor(bh) {
    try {
      ...
      this.page.userInfor = outputVariables.local.userInfor;

      bh = this.getUserImage(bh);
      this.sd_x9Fl18em53PzBaJ6(bh);
      //appendnew_next_getUserInfor
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_x7q3a2YhyEUDlLY4');
    }
  }

  async getUserImage(bh) {
    try {
      const api_serviceInstance: api_service =
        this.__page_injector__.get(api_service);

      let outputVariables = await api_serviceInstance.getPic();
      this.page.image = outputVariables.local.userImage;

      bh = this.sd_sfuEkB8gmGQbT1qu(bh);
      //appendnew_next_getUserImage
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_nObXmb78xJKhsXWN');
    }
  }

  async sd_sfuEkB8gmGQbT1qu(bh) {
    try {
      const api_serviceInstance: api_service =
        this.__page_injector__.get(api_service);

      let outputVariables = await api_serviceInstance.getUserPic();
      bh.image = outputVariables.local.userPicURL;

      bh = this.sd_V4i6zcu0GGDoXWUc(bh);
      //appendnew_next_sd_sfuEkB8gmGQbT1qu
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_sfuEkB8gmGQbT1qu');
    }
  }

  sd_V4i6zcu0GGDoXWUc(bh) {
    try {
      const page = this.page;
      console.log('User from a service: ', page.userInfor);
      console.log('User page image: ', page.image);
      console.log('User image for activation: ', bh.image);
      ...
      //appendnew_next_sd_V4i6zcu0GGDoXWUc
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_V4i6zcu0GGDoXWUc');
    }
  }

  sd_x9Fl18em53PzBaJ6(bh) {
    try {
      console.log(new Date().toLocaleTimeString(), this.page.userInfor);
      //appendnew_next_sd_x9Fl18em53PzBaJ6
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_x9Fl18em53PzBaJ6');
    }
  }
  ...
  async sd_SYRWXoozvB0Z6fFv(bh) {
    try {
      ...
      if (page.routeData.isRegister) {
        ...
        page.formData.append('name', page.userInfor.ID);

        page.formData1 = new FormData();
        page.formData1.append('name', page.userInfor.POR);

        page.formData2 = new FormData();
        page.formData2.append('name', page.userInfor.Pic);
      }
      ...
      page.detectVideoFace.forEach((singleFace: any) => {
        ...
        if (distance < page.similiarityValue) {
          ...
          if (page.routeData.isRegister) {
            ...
            if (page.num < 1) {
              this.storeUserInfor('registration', page.userInfor.userInfor);

              this.storeUserInfor('upload-ID', page.formData);
              this.storeUserInfor('upload-proof', page.formData1);
  ...
  async sd_758Wv0M4eHNK7wFL(bh) {
    try {
      const api_serviceInstance: api_service =
        this.__page_injector__.get(api_service);

      let outputVariables = await api_serviceInstance.uploadFile(
        bh.input.url,
        bh.input.infor
      );

      //appendnew_next_sd_758Wv0M4eHNK7wFL
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_758Wv0M4eHNK7wFL');
    }
  }
