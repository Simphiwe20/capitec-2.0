...
export class verify_identityComponent implements AfterViewInit {
  ...
  async setInfor(bh) {
    try {
      ...
      let outputVariables = await api_serviceInstance.setInfor(bh.user);

      bh = this.setPic(bh);
      //appendnew_next_setInfor
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_I3Zo3DarA8GU5a1J');
    }
  }
  ...
  async setPic(bh) {
    try {
      ...
      let outputVariables = await api_serviceInstance.setPic(
      ...
      return this.errorHandler(bh, e, 'sd_W6N6qHGZm9qAd6pi');
    }
  }
